...
CommandManager& CommandManager::getInstance(IOHandler& io, UserManager& userManager, ProductManager& productManager, IDataHandler& dataHandler, IFormatter& formatter) {
    ...
    static CommandManager instance(io, userManager, productManager, dataHandler, formatter);
    return instance;
}
...
CommandManager::CommandManager(IOHandler& io, UserManager& userManager, ProductManager& productManager, IDataHandler& dataHandler, IFormatter& formatter) : io(io) {
    ...
    commands["POST"]      = new PostCommand(userManager, productManager, io, dataHandler, formatter);
    commands["PATCH"]     = new PatchCommand(userManager, productManager, io, dataHandler, formatter);
    commands["DELETE"]     = new DeleteCommand(userManager, productManager, io, dataHandler, formatter);
    commands["GET"] = new RecommendCommand(userManager, productManager, io, formatter);
